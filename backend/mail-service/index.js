...
const encryptUtils = require('../utils/encrypt/index')
...
exports.sendMailConfirm = (user, fromEmail = FROM_EMAIL) => {
  ...
  let emailToken = await createEmailToken(user);
...
exports.sendMailConfirmChangePassword =  (user, fromEmail = FROM_EMAIL) => { 
  let expired = (TOKEN_LIFETIME / 3600);
  let emailToken = await createEmailToken(user);
  let confirmUrl=url+"/public-user/verify-changed-password?emailToken="+emailToken;
